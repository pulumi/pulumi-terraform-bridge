...
func TestTokensSingleModule(t *testing.T) {
	...
	err := info.ComputeDefaults(tfbridge.ProviderInfoStrategy{rTokens, dTokens})
	...
	err = info.ComputeDefaults(tfbridge.ProviderInfoStrategy{Resources: rTokens})
	require.NoError(t, err)
...
func TestUnmappable(t *testing.T) {
	...
	err := info.ComputeDefaults(tfbridge.ProviderInfoStrategy{Resources: strategy})
