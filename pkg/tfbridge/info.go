...
type ProviderInfo struct {
	...
	PulumiSchema            *PulumiSchemaInfo          // optional information for augmented Pulumi Schema generation.
...
// PulumiSchemaInfo contains optional overrides for Pulumi schema generation.
type PulumiSchemaInfo struct {
	ExtraTypes   map[string]pschema.ComplexTypeSpec // a map of Pulumi token to schema type for overlaid types.
	ModuleFormat *ModuleFormatInfo                  // optional override for the module format
}

type ModuleFormatInfo struct {
	Format       string                             // the module format regex
	NewTypeToken func(pkg, mod, name string) string // the function that will be called to create tokens for object types
}
