...
func TestToInstanceState(t *testing.T) {
	...
	assert.Equal(t, state.(v1InstanceState).tf.Attributes, map[string]string{
	...
	assert.NoError(t, err)
	assert.Equal(t, expected, state.(v1InstanceState).tf.Attributes)
}
...
func TestObjectFromInstanceDiff(t *testing.T) {
	...
	s.diff = &terraform.InstanceDiff{
		Attributes: map[string]*terraform.ResourceAttrDiff{
			...
			"resource_set_property_value.0.labels.0": {
				New:         UnknownVariableValue,
				NewComputed: true,
			},
		},
