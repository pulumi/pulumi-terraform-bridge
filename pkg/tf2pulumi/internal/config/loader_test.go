...
func TestLoadFile_resourceArityMistake(t *testing.T) {
	...
	path := filepath.Join("testdata", "resource-arity-mistake.tf")
	expected := "Error loading " + path + ": position 2:10: resource must be followed by exactly two strings, a type and a name"
...
func TestLoadFile_dataSourceArityMistake(t *testing.T) {
	...
	path := filepath.Join("testdata", "data-source-arity-mistake.tf")
	expected := "Error loading " + path + ": position 2:6: 'data' must be followed by exactly two strings: a type and a name"
	if err.Error() != expected {
		t.Fatalf("expected:\n%s\ngot:\n%s", expected, err)
	}
}
