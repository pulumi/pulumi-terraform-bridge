...
func ProviderOptionality() tfbridge.ProviderInfo {
	pkg := "optionality"
	mod := "index"
	...
	resource := func(res string) tokens.Type {
		fn := string(unicode.ToLower(rune(res[0]))) + res[1:]
		return typ(mod+"/"+fn, res)
	}

	return tfbridge.ProviderInfo{
		P:        shimv2.NewProvider(testproviderdata.ProviderOptionality()),
		Name:     "optionality",
		License:  "Apache-2.0",
		Homepage: "https://pulumi.io",
		Resources: map[string]*tfbridge.ResourceInfo{
			"res": {Tok: resource("Res")},
		},
	}
}
