...
func splitByMarkdownHeaders(content string, level int) [][]string {
	...
	headers := []int{}
	parse.WalkNode(gm.Parser().Parse(text.NewReader(bytes)), func(heading *ast.Heading) {
	...
	for _, section := range splitStringsAtIndexes(content, headers) {
...
func (p *tfMarkdownParser) readSupplementaryExamples() ([]byte, error) {
...
func promoteHeadersWithCode(node ast.Node) {
	var headers []*ast.Heading
	err := ast.Walk(node, func(n ast.Node, entering bool) (ast.WalkStatus, error) {
		switch n := n.(type) {
		case *ast.Heading:
			switch {
			// Push onto the stack if we might want to modify this header
			case entering && n.Level == 4:
				headers = append(headers, n)
			// Pop from the stack if we are exiting this header
			case !entering && len(headers) > 0 && headers[len(headers)-1] == n:
				headers = headers[:len(headers)-1]
			}
			return ast.WalkContinue, nil
		case *ast.CodeBlock:
			if len(headers) > 0 {
				headers[len(headers)-1].Level = 3
			}
			return ast.WalkContinue, nil
		default:
			return ast.WalkContinue, nil
	...
	contract.AssertNoErrorf(err, "impossible - the walker never returns an error")
	contract.Assertf(len(headers) == 0, "We should have exited every header we entered")
}

type exampleTransformer struct {
	replace *struct{ content []byte }
}

func (e exampleTransformer) Transform(node *ast.Document, reader text.Reader, pc parser.Context) {
	if e.replace == nil {
		e.reformatExamples(node, reader, pc)
	} else {
		e.replaceExamples(node, reader, pc)
	}
}

var exampleHeaderRegexp = regexp.MustCompile(`(?i)^(Example Usage\s*)(?:(?:(?:for|of|[\pP]+)\s*)?(.*?)\s*)?$`)

func (e exampleTransformer) reformatExamples(node *ast.Document, reader text.Reader, pc parser.Context) {
	// First, find the canonical "Example Usage" section and all other docs sections.
	var canonicalExampleSection *section.Section
	var nestedExampleSections []*section.Section
	err := ast.Walk(node, func(n ast.Node, entering bool) (ast.WalkStatus, error) {
		if entering {
			return ast.WalkContinue, nil
		}
		node, ok := n.(*section.Section)
		if !ok || node.FirstChild().(*ast.Heading).Level != 2 {
			return ast.WalkContinue, nil
		}

		headerText := node.FirstChild().Text(reader.Source())
		matches := exampleHeaderRegexp.FindSubmatch(headerText)
		if len(matches) == 0 {
			return ast.WalkContinue, nil
		}

		if len(matches[1]) == len(headerText) && canonicalExampleSection == nil {
			// We have found a canonical section, so set that and keep going
			canonicalExampleSection = node
			return ast.WalkContinue, nil
		}
		...
		if len(matches[2]) != 0 {
			...
			header.AppendChild(header, ast.NewString(matches[2]))
		}

		nestedExampleSections = append(nestedExampleSections, node)
		return ast.WalkContinue, nil
	})
	contract.AssertNoErrorf(err, "impossible - the walker doesn't return an error")

	// If there are no nested sections, we don't want to create an empty "## Example
	// Usage" section.
	if len(nestedExampleSections) == 0 {
		return
	}

	// If no canonical section exists, create it at the end of the document.
	if canonicalExampleSection == nil {
		h2 := ast.NewHeading(2)
		h2.AppendChild(h2, ast.NewString([]byte("Example Usage")))
		section := section.New(h2)
		node.AppendChild(node, section)
		canonicalExampleSection = section
	}

	// Append all non-canonical sections to the canonical section, setting the level
	// to 3.
	for _, section := range nestedExampleSections {
		header := section.FirstChild().(*ast.Heading)
		header.Level = 3

		canonicalExampleSection.AppendChild(canonicalExampleSection, section)
	}

	promoteHeadersWithCode(canonicalExampleSection)
}

// replaceExamples removes any sections that have a header that contains "Example Usage",
// then appends e.replace.content to the end of the document.
func (e exampleTransformer) replaceExamples(node *ast.Document, reader text.Reader, pc parser.Context) {
	// We remove existing examples
	skipExamples := func(headerText string) bool {
		return strings.Contains(headerText, "Example Usage")
	}
	sectionSkipper{skipExamples}.Transform(node, reader, pc)

	node.AppendChild(node, ast.NewString(e.replace.content))
}
...
func (p *tfMarkdownParser) parse(tfMarkdown []byte) (entityDocs, error) {
	...
	tfMarkdown = bytes.ReplaceAll(tfMarkdown, []byte("\r\n"), []byte("\n"))

	// Replace redundant comment.
	tfMarkdown = bytes.ReplaceAll(tfMarkdown, []byte("<!-- schema generated by tfplugindocs -->"), nil)

	var exampleTransformer exampleTransformer
	if p.info != nil && p.info.GetDocs() != nil && p.info.ReplaceExamplesSection() {
		b, err := p.readSupplementaryExamples()
		if err != nil {
			return entityDocs{}, err
		}
		exampleTransformer.replace = &struct{ content []byte }{b}
	}
...
func (p *tfMarkdownParser) parseSection(h2Section []string) error {
	...
	header := h2Section[0]
	if rest, ok := strings.CutPrefix(header, "## "); ok {
		header = rest
	} else {
		if strings.HasPrefix(header, "---") {
			sectionKind = sectionFrontMatter
		} else {
			h2Section = append([]string{""}, h2Section...)
		}
		header = ""
	}
