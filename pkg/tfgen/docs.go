...
func splitByL2Headers(text string) [][]string {
	...
	parseDoc(bytes).Walk(func(node *bf.Node, entering bool) bf.WalkStatus {
		...
		if node.Type != bf.Heading || node.HeadingData.Level != 2 || node.HeadingData.IsTitleblock {
			return bf.GoToNext
		}
		fmt.Println(string(node.FirstChild.Literal))
		var foundOne bool
		...
		contract.Assertf(foundOne, "Failed to find source location of a header")
		return bf.GoToNext
	})
	old := groupLines(strings.Split(text, "\n"), "## ")
	if len(old) != len(headers)+1 {
		panic(fmt.Sprintf("UNEXPECTED: old = %d, new is %d", len(old), len(headers)))
	}
	return old
}
...
func (p *tfMarkdownParser) parse(tfMarkdown []byte) (entityDocs, error) {
	...
	sections := splitByL2Headers(markdown)
