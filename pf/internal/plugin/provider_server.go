...
func NewProviderServerWithContext(
	...
	p := &providerServer{plugin.NewProviderServer(providerThunk{NewProvider(provider)})}
	...
	contract.AssertNoErrorf(err, "False configure call should never read out of the package, so should not fail.")
	return p
}
