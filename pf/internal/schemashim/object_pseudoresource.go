...
func isObject(t attr.Type) (basetypes.ObjectTypable, bool) {
	tt, ok := t.(basetypes.ObjectTypable)
	return tt, ok
}
...
func newObjectPseudoResource(t basetypes.ObjectTypable, nestedAttrs map[string]pfutils.Attr) *objectPseudoResource {
	...
	if !ok {
		panic("t basetypes.ObjectTypable should produce a tftypes.Object in t.TerraformType()")
	}
	var attrs []string
...
func (r *objectPseudoResource) attrs() map[string]pfutils.Attr {
	return r.nestedAttrs
}
...
func (r *objectPseudoResource) GetOk(key string) (shim.Schema, bool) {
	...
	// and recurse using attrSchema. This information may be coming out of band from the ObjectTypeable value
	// itself when using blocks, see TestCustomTypeEmbeddingObjectType.
	...
	for _, a := range r.allAttrNames {
		if key == a {
			panic("[pf/internal/schemashim] Attribute %q should be defied was not found, this should never happen")
		}
	}
