...
func TestCheckConfig(t *testing.T) {
	...
	t.Run("invalid_config_value", func(t *testing.T) {
		...
		autogold.Expect("`testprovider:requiredprop` is not a valid configuration key for the testprovider provider. If the key is not intended for the provider, please choose a different namespace from `testprovider:`.").Equal(t, resp.Failures[0].Reason)
		// Explicit provider.
		resp, err = provider.CheckConfig(ctx, &pulumirpc.CheckRequest{
	...
	t.Run("levenshtein_correction", func(t *testing.T) {
		...
		require.Equal(t, 1, len(resp.Failures))
		//nolint:lll
		autogold.Expect("`testprovider:cofnigValue` is not a valid configuration key for the testprovider provider. Did you mean `testprovider:configValue`? If the key is not intended for the provider, please choose a different namespace from `testprovider:`.").Equal(t, resp.Failures[0].Reason)
	})
